...
class PersonalInfoLogic extends GetxController {
  ...
  void _queryUserFullInfo() async {
    ...
    final list = await LoadingView.singleton.wrap(
      asyncFunction: () => ChatApis.getUserFullInfo(userIDList: [userID]),
    );
  ...
  String? get phoneNumber => clientConfigLogic.replacePhoneNumber(userFullInfo.value.phoneNumber);

  String? get email => IMUtils.emptyStrToNull(userFullInfo.value.email) ?? '-';

  void clickPhoneNumber() {
    if (clientConfigLogic.showPhoneNumber && userFullInfo.value.phoneNumber != null) {
      _callSystemPhone(userFullInfo.value.phoneNumber);
    }
  }
