...
class UserProfilePanelLogic extends GetxController {
  ...
  void onInit() {
    ...
    // For search flow, don't use cached data (wait for fresh API response)
    // For other flows (friend profile, group member), use cache immediately
    final isFromSearch = Get.arguments['isFromSearch'] ?? false;
    isDataLoaded.value = !isFromSearch;
  ...
  void _getUsersInfo() async {
    ...
    // Mark data as loaded so UI can render consistently
  ...
  void deleteFromFriendList() async {
    ...
    if (confirm == true) {
      // Navigate away immediately to prevent UI flicker
      ...
      // Process unfriend in background
      try {
